# 
# This script makes a graph from PathwayMatcher output.
#
...
matchingCategoryColors <- c("blue3", "blue3", "green3")


# Functions

mergeExternalInternal <- function(externalGraph, internalGraph) {
  
  
  reactomeGraph <- union(internalGraph, externalGraph)
  
  source1 <- get.edge.attribute(reactomeGraph, "source_1")
  source2 <- get.edge.attribute(reactomeGraph, "source_2")
  sourceMerge <- ifelse(!is.na(source1), source1, source2)
  
  type1 <- get.edge.attribute(reactomeGraph, "type_1")
  type2 <- get.edge.attribute(reactomeGraph, "type_2")
  typeMerge <- ifelse(!is.na(type1), type1, type2)
  
  reactomeGraph <- set.edge.attribute(graph = reactomeGraph, name = "source", value = sourceMerge)
  reactomeGraph <- set.edge.attribute(graph = reactomeGraph, name = "type", value = typeMerge)
  
  reactomeGraph <- remove.edge.attribute(reactomeGraph, "source_1")
  reactomeGraph <- remove.edge.attribute(reactomeGraph, "source_2")
  reactomeGraph <- remove.edge.attribute(reactomeGraph, "type_1")
  reactomeGraph <- remove.edge.attribute(reactomeGraph, "type_2")
...
internalEdgesAccessions <- read.table("C:\\Projects\\Francisco\\PathwayMatcher\\cAMP\\Accessions\\internalEdges.tsv", header = T, sep = "\t", quote = "", comment.char = "", stringsAsFactors = F)
internalEdgesProteoforms <- read.table("C:\\Projects\\Francisco\\PathwayMatcher\\cAMP\\Proteoforms\\internalEdges.tsv", header = T, sep = "\t", quote = "", comment.char = "", stringsAsFactors = F)

edgesProteoforms <- read.table("C:\\Projects\\Francisco\\PathwayMatcher\\all\\proteoformInternalEdges.tsv", header = T, sep = "\t", quote = "", comment.char = "", stringsAsFactors = F)
...
proteoformAccessions <- unique(sapply(proteoforms, getAccession))

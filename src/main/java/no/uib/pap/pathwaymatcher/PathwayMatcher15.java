...
public class PathwayMatcher15 {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            switch (inputType) {
                ...
                case GENES:
                    searchResult = Search.searchWithGene(input, iReactions, iPathways, imapGenesToProteins, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithGene(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case ENSEMBL:
                    searchResult = Search.searchWithEnsembl(input, iReactions, iPathways, imapEnsemblToProteins, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithEnsembl(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case UNIPROT:
                    searchResult = Search.searchWithUniProt(input, iReactions, iPathways, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithUniProt(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case PROTEOFORMS:
                    searchResult = Search.searchWithProteoform(input, matchType, margin, iReactions, iPathways, imapProteinsToProteoforms, imapProteoformsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithProteoform(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteoformsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case RSIDS:
                    searchResult = Search.searchWithRsId(input, iReactions, iPathways, imapRsIdsToProteins, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithUniProt(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case CHRBPS:
                    searchResult = Search.searchWithChrBp(input, iReactions, iPathways, imapChrBpToProteins, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithUniProt(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case VCF:
                    searchResult = Search.searchWithVCF(input, iReactions, iPathways, imapChrBpToProteins, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithUniProt(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case PEPTIDES:
                    searchResult = Search.searchWithPeptide(input, iReactions, iPathways, imapProteinsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithUniProt(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteinsToReactions.keySet().size(), searchResult.getKey());
                    break;
                case MODIFIEDPEPTIDES:
                    searchResult = Search.searchWithModifiedPeptide(input, matchType, margin, iReactions, iPathways, imapProteinsToProteoforms, imapProteoformsToReactions, imapReactionsToPathways, imapPathwaysToTopLevelPathways, commandLine.hasOption("tlp"));
                    outputSearchWithProteoform(searchResult.getKey());
                    analysisResult = Analysis.analysis(iPathways, imapProteoformsToReactions.keySet().size(), searchResult.getKey());
                    break;
                default:
    ...
    private static void writeAnalysisResult(TreeSet<Pathway> key) {
        try {
    ...
    private static void outputSearchWithGene(List<String[]> searchResult) throws IOException {
        ...
        outputSearch.write("GENE" + separator
                + "UNIPROT" + separator
                + "REACTION_STID" + separator
                + "REACTION_DISPLAY_NAME" + separator
                + "PATHWAY_STID" + separator
    ...
    private static void outputSearchWithEnsembl(List<String[]> searchResult) throws IOException {
        ...
        outputSearch.write("ENSEMBL" + separator
                + "UNIPROT" + separator
                + "REACTION_STID" + separator
                + "REACTION_DISPLAY_NAME" + separator
                + "PATHWAY_STID" + separator
    ...
    private static void outputSearchWithUniProt(List<String[]> searchResult) throws IOException {
        ...
        outputSearch.write("UNIPROT" + separator
                + "REACTION_STID" + separator
                + "REACTION_DISPLAY_NAME" + separator
                + "PATHWAY_STID" + separator
                + "PATHWAY_DISPLAY_NAME");
    ...
    private static void outputSearchWithProteoform(List<String[]> searchResult) throws IOException {
        ...
        outputSearch.write("PROTEOFORM" + separator
                + "REACTION_STID" + separator
                + "REACTION_DISPLAY_NAME" + separator
                + "PATHWAY_STID" + separator
                + "PATHWAY_DISPLAY_NAME");
